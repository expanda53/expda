...
str = 'kiadkesz_kovsor '..fejazon..' '..hkod..' ' ..cikk..' '..irany..' '..kezelo
...
if (result=='0') then
    ...
    if (ujhkod~=hkod or ujhkod~=akthkod) then
      ...
      ui:executeCommand('hideobj','cap_drb;cap_edrb2;cap_ean;eean;button_kovetkezo','')
      ...
      ui:executeCommand('hideobj','cap_drb;cap_edrb2','')      
      ui:executeCommand('setfocus','eean','')      
    end

    ui:executeCommand('valuetohidden','ldrb', t[1]['DRB'])
    ui:executeCommand('valuetohidden','edrb2', t[1]['DRB'])
else
  if (result~='-1') then
    if (irany=='1') then
        if (cikk=='.') then
            ...
            ui:executeCommand('valuetohidden','eean', '')
            ui:executeCommand('hideobj','cap_drb;cap_edrb2;cap_hkod;lhkod;cap_ean','')      
            ui:executeCommand('toast','Nincs több kiszedendő tétel!','')
            ui:executeCommand('startlua','kiadkesz/showreview.lua','')
        else
          ui:executeCommand('startlua','kiadkesz/kovetkezo_click.lua', fejazon..' . . 1')
        end
    elseif (irany=='0') then
        if (result=='2') then
